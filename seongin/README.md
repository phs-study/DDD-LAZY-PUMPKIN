...
# 8 애그리거트 트랜잭션 관리
# 8.1 애그리거트와 트랜잭션
- 애그리거트의 일관성을 보장해야 한다. (쓰레드 변수에 진입한 경우, 동시성을 제어해 접근을 먼저 획득한 쓰레드를 기다리는 등의 조치가 필요)
- 선점 잠금(Pessimistic Lock)
- 비선점 잠금(Optimistic Lock)

# 8.2 선점 잠금
> 선점한 쓰레드가 애그리거트의 사용이 끝날 때까지 다른 쓰레드에서의 접근을 막는다.
- 보통 DBMS가 제공하는 행단위 잠금을 사용해서 구현한다. (ex. 오라클의 for update 쿼리 등)
- JPA EntityManaager는 LockModeType을 PESSIMISTIC_WRITE로 전달할 수 있는 인터페이스를 제공한다.
# 8.2.1 선점 잠금과 교착 상태
- 두 쓰레드가 경쟁 상태(Race condition)에 들어가면 교착 상태(DeadLock)에 빠진다.
    1. 쓰레드 1이 A를 잠근다.
    2. 쓰레드 2가 B를 잠근다.
    3. 쓰레드 1이 B에 접근한다.
    4. 쓰레드 2가 A에 접근한다.
> 잠금 최대 대기 시간을 설정하여 예방한다.

# 8.3 비선점 잠금
> 선점 잠금으로 동시성 충돌을 예방하더라도 구분된 트랜잭션에서 발생하는 비정상적 동작을 막을 수는 없다.
> 아래와 같은 상황에서는 동시성 문제가 없었으나 물건은 오발송된다.
1. 운영자가 주문 정보 조회
2. 고객이 주문 정보 변경 (배송지 변경)
3. 운영자가 배송 상태 변경
> 비선점 잠금은 실제 데이터를 변경하는 시점에 변경가능여부를 확인하는 방식이므로 위와 같은 상황을 예방할 수 있다.
# 8.3.1 강제 버전 증가
루트 애그리거트가 아닌 다른 엔티티의 변경이 발생하더라도 루트 애르리거트 조회 시, 버전을 강제로 증가시켜 비선점 잠금을 올바르게 구현한다.
# 8.4 오프라인 선점 잠금
- 트랜잭션 범위 수준에서 벗어난 강력한 잠금을 처리한다.
    - 특정 트랜잭션에서 오프라인 자금
    - 특정 트랜잭션에서 오프라인 잠금 해제
> 오프라인 잠금 중에는 락을 얻을 수 없다. 강력한 잠금인 만큼 오프라인 잠금 해제 트랜잭션이 호출되지 않으면 잠금이 풀리지 않는다.
> <br> 따라서, 오프라인 선점 잠금은 잠금 유효 시간을 필수적으로 지정해야한다.
> <br> 또한, 트랜잭션 중에 있음을 확인하고 잠금 유효 시간을 지속적으로 업데이트해주는 로직도 함께 필요하다.

# 9.1 도메인 모델과 경계
> 처음부터 도메인을 완벽하게 표현하는 단위 모델을 만드는 것은 불가능하다. 
> 하위 도메인마다 단위 모델을 만들어줘야 올바른 도메인 모델을 개발할 수 있다. 이 때,
> 여러 하위 도메인의 모델이 섞이지 않도록 각 모델을 명시적으로 구분하는 경계를 가져야 한다.

모델은 특정 컨텍스트(문맥) 에서 완전한 의미를 갖는다. 같은 제품이라도 카탈로그 컨텍스트와 재고 컨텍스트에서 의미가 서로 다르다.
이렇게 구분되는 경계를 갖는 컨텍스트를 DDD에서는 바운디드 컨텍스트(Bounded Context)라고 부른다.

# 9.2 바운디드 컨텍스트
한 개의 바운디드 컨텍스트는 논리적으로 한 개의 모델을 갖는다. 바운디드 컨텍스트는 용어를 기준으로 구분하며 도메인 모델이 구현되는 곳이다.
이상적으로는 하위 도메인과 바운디드 컨텍스트가 일대일 관계를 가지는 것이 좋으나, 조직의 구조와 용어의 구분이 명확하지 않은 시점 등의 이유로
다양한 관계(1:N 혹은 N:1)가 나타난다.
> 바운디드 컨텍스트마다 표현하는 하위 도메인은 같은 용어라도 달라질 수 있음을 꼭 유의하라.
> EX) 회원 BC에서 Member는 애그리거트 루트이지만 주문 BC에서는 Orderer로 밸류가 된다.
> EX) 캍탈로그 BC에서 Producer는 Category와 연관을 갖지만 재고 BC에서 Product는 Category와 연관이 없다.
...
> 복잡한 도메인 로직이 필요하지 않은 경우 도메인 영역이 없이 단순 CURD방식으로 (DBMS > DAO > 서비스 > 표현 영역)와 같은 바운디드 컨텍스트로도 설계할 수 있다.
 
> 바운디드 컨텍스트는 서로 다른 구현 기술을 사용할 수도 있고 꼭 UI를 가지고 있을 필요도 없다. 요구 사항에 맞게끔 자유롭게 구현해도 된다.

# 9.4 바운디드 컨텍스트 간 통합
REST API(직접) 혹은 메세지 큐(간접)을 이용해 두 바운디드 컨텍스트를 통합할 수 있다. 이 때, 하위 도메인의 식별자를 간접 참조로 활용하여 
인터페이싱 후에는 각 BC에 맞는 하위 도메인으로 변환하여 사용할 수 있도록 한다.

# 9.5 바운디드 컨텍스트 간 관계
API 공급자와 API 고객의 관계에서 공급자는 다양한 고객의 형태를 반영하여 서비스의 일관성을 유지해야한다.
이러한 서비스를 공개 호스트 서비스(Open Host Service)라고 한다.

공개 호스트 서비스의 대표적인 예는 검색이다. 블로그, 카페, 게시판과 같은 API 고객 서비스는 검색 API 공급자에 의해 통합된다.
검색 서비스는 상류 컴포넌트이며 다른 서비스들은 하류 컴포넌트로 분류할 수 있다.
검색 서비스 개발팀과 타 개발팀은 공개 호스트 서비스와 모델 변환기를 활용하여 결합도를 최소화하며 개발해야한다. 

반대로, 두 바운디드 컨텍스트가 같은 모델을 공유하는 경우도 있다. 이를 공유 커널(Shared Kernel)이라고 부른다.
두 팀이 긴밀하고 밀접한 관계를 유지하며 개발을 해야만 장점을 극대화할 수 있다.

마지막으로, 독립 방식(Separate Way)가 있다. 두 바운디드 컨텍스트 간에 통합이 없는 방식이다.
시스템의 개입이 없으므로 수동으로 통합이 이루어진다. 규모가 커질수록 한계가 명확한 방식이다.

# 9.6 컨텍스트 맵
> 시스템의 전체 구조를 포함한 바운디드 컨텍스트 간의 관계를 표시한다.

조직 입장에서 사업 핵심 도메인을 파악하고 집중해야할 도메인을 파악하는데 도움이 된다.

# 10.1 시스템 간 강결합 문제
> 외붐 시스템 연계 기능에서 아래와 같은 문제가 발생할 수 있다.
1. 트랜잭션 처리 (롤백 or 재시도)
2. 시스템 응답 시간이 외부 서비스 응답 시간에 종속
3. 그 외에, 서비스를 도메인 객체 전달할 경우 비즈니스 로직이 섞이는 문제도 발생(주문 로직과 환불 로직이 Order 도메인에 섞임)

> 위 문제는 BC 간의 강결합(high coupling) 때문이다. 이러한 문제를 해결하기 위해 이벤트를 사용한다.

# 10.2 이벤트 개요
> 이벤트 : 과거에 벌어진 어떤 것
- 이벤트의 구성 요소 : 이벤트, 이벤트 주체, 이벤트 디스패처(퍼블리셔), 이벤트 구독자
  - 도메인 모델에서 이벤트 생성 주체는 엔티티, 밸류, 도메인 서비스와 같은 도메인 객채이다.
- 이벤트의 용도
  1. 트리거(Trigger) : 도메인의 상태 변경에 따른 후처리 (주문 취소 트리거 -> 환불 처리)
  2. 동기화 : 이종 시스템 간의 데이터 동기화에 사용 (외부 서비스와 데이터 동기화)
- 이벤트 장점
  - 서로 다른 도메인 로직이 섞이지 않도록 한다.
  - 기능 확장이 용이하다.
# 10.4 동기 이벤트 처리 문제
> 강결합 문제를 해소하더라도 외부 서비스에 여전히 영향을 받는다.
# 10.5 비동기 이벤트 처리
1. 로컬 핸들러 비동기 실행
   - 별도 스레드로 이벤트 핸들러를 실행
2. 메시징 시스템을 이용한 비동기 구현
   - 카프카 혹은 래빗 같은 메시징 시스템을 사용한다. (프로세스 단위 격리)
3. 이벤트 저장소를 이용한 비동기 처리
   - DB에 이벤트를 저장한 후 별도 프로그램(포워더)을 이용해 이벤트 핸들러로 전달한다.
     - 도메인과 이벤트가 같은 물리적 공간(DB)에 저장되어 트랜잭션이 공유된다.
# 10.6 이벤트 적용 시 추가 고려 사항
1. 이벤트 발생 주체를 추적할 것 인가 
    - 이벤트 발생 주체에 대한 정보를 가질 것 인가 ?
2. 포워더에서 전송 실패를 얼마나 허용할 것 인가
    - 3회 실패했다면 해당 이벤트는 생략하는 등의 정책 필요 
3. 이벤트 손실에 대한 정책
    - 비동기 처리에서 이벤트 처리에 실패할 경우 이벤트 유실 발생 
4. 이벤트의 순서 보장 정책
    - 발생 순서대로 외부 시스템에 전달이 필요할 경우 저장소를 사용
5. 이벤트 재처리에 대한 정책
    - 동일 이벤트를 재처리해야하는가 ? 무시할 것 인가?
> 멱등성을 가지는 이벤트는 중복 발생이나 처리에 대한 부담을 줄여준다.

> 이벤츠 실패 처리와 트랜잭션 실패 처리를 고려해야한다.
> 트랜잭션이 성공할 때만 이벤트 핸들러를 실행하는 방법이 있다.
> 또한, 이벤트 저장소로 DB를 사용해도 동일한 효과를 볼 수 있다.

# 11.1 단일 모델의 단점
> 조회와 변경에 대해 단일 모델을 사용하게 되면 쿼리 최적화 기능을 사용하기 어렵다.
> 이러한 문제를 해결하기 위해, 상태 변경을 위환 모델과 조회를 위한 모델을 분리하는 것이다.

# 11.2 CQRS
> 상태를 변경하는 범위와 조회하는 범위가 정확하게 일치하지 않기 때문에 단일 모델로 두 종류의 기능을 구현하면 모델이 불필요하게 복잡해진다.
> CQRS(Command Query Responsibility Segregation) 패턴으로 상태 변경 명령(Command)와 상태 조회(Query)를 위한 모델을 분리한다.

> 명령 모델과 조회 모델을 서로 다른 기술을 사용해 구현해도 무방하나 동기화에 신경써줘야 한다.

- CQRS 장단점
  - 장점
    - 복잡한 도메인에서 명령 모델과 조회 모델을 구분함으로써, 상태 변경 로직에 집중할 수 있다.
    - 조회 특화 쿼리를 마음대로 사용할 수 있다.
    - 조회 단위로 캐시 기술을 적용할 수 있다.
    - NoSQL과 같은 조회 전용 저장소를 사용해 조회 처리량을 늘릴 수 있다.
